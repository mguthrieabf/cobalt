...
from rbac.models import RBACModelDefault, RBACAppModelAction


class Command(BaseCommand):
    def CreateRBACDefault(self, app, model):
        if not RBACModelDefault.objects.filter(app=app, model=model).exists():
            r = RBACModelDefault(app=app, model=model, default_behaviour="Allow")
            r.save()
            self.stdout.write(
            ...
            self.stdout.write(
                self.style.SUCCESS("RBACModelDefault already exists - ok")
            )
